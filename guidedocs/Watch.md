...
vue2에서는 감시할 속성을 함수형태가 아닌 객체 형태로 정의하며, 내부에 handler라는 옵션을 통해 감시후 실행할 핵심 로직을 담은 콜백 함수를 정의하여 함께 사용해야 하며, vue3에서는 세번째 인자에 객체 형태로 정의하여 사용한다.
...
앞서 말했듯, 실행중에도 만들고 제거할수 있다는것은 특정 method 호출 시점에도 watch를 등록하여  
해당 method가 실행되는 동안에만 감지할 수 있도록 처리할 수도 있다.  
제거만 잘 해주면 된다.  
...
watch(count, ...) 처럼 ref 변수명 자체를 감시 대상으로 사용하거나 혹은 watch(() => count.value, ...)와 같이 value속성으로 접근하되,  
함수로 한번 감싸줘야 한다.
